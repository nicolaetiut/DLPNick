...
def results(request, quest_id):
    ...
    for result in q.result_set.all():
        if res < result.upper_limit:
            ...
            break
    if final_result:    
        answer_less = quest_algorithm.check_result(q, current_answers, final_result.upper_limit + 1 - res)
        answer_more = quest_algorithm.check_result(q, current_answers, final_result.upper_limit + 1 - res)
    return render_to_response('results.html', {'quest': q,
                                               'result': final_result,
    ...
    if current_q and current_q == quest_id:
        if current_p:
            if int(current_p) > int(page_id):
                return HttpResponseRedirect(reverse('questionnaires.views.go_to_page', args=(current_q, current_p,)))
        else:
            if int(page_id) != 1:
                return HttpResponseRedirect(reverse('questionnaires.views.go_to_page', args=(current_q, '1',)))
    else:
        request.session["current_quest_id"] = quest_id
        request.session["current_answers"] = {}
        pid = page_id
        if int(page_id) != 1:
            pid = 1
        return HttpResponseRedirect(reverse('questionnaires.views.go_to_page', args=(q.id, pid,)))

      
    return render_to_response('page.html', {'quest': q, 'page': page}, context_instance=RequestContext(request))        
...
def validate(request, quest_id, page_id):
    ...
    if int(page_id) == len(list(q.page_set.all())):
        return HttpResponseRedirect(reverse('questionnaires.views.results', args=(q.id,)))
    
    return HttpResponseRedirect(reverse('questionnaires.views.go_to_page', args=(q.id, (int(page_id) + 1))))
